...
把任意关键词/主题变成 `E:\obsidian\rein\<主题>\` 下的一套**"先总后细"的分层知识库**，流程 = **发散（多视角）→ 细化（逐类深挖）→ 建库（写文件）**。本 Skill 是豆包（Doubao/MainAgent）侧的完整工作流规范，直接在本机执行，不依赖 OpenClaw。
...
2. **意图澄清（P0，强制）**：检测关键信息是否充分，不足则先反问 1-3 个高价值问题（附"为什么问"），不猜、不直接建库：
   ...
   - `E:\obsidian\rein` 下是否已建过相关库？（增量 or 新建）
   - 若用户已给足信息则跳过反问。
3. **查已有库（P0）**：扫描 `E:\obsidian\rein\` 根目录，若已存在同名/相似主题 → 提示用户增量补充 or 另建（默认增量）。
4. 用户确认"开始/确认"后才进入发散。
...
1. **文件直写**：Vault 是纯 Markdown 文件夹，直接用文件工具操作 `E:\obsidian\rein\`，不依赖 Obsidian App/插件。
2. **每个主题 = 独立 vault**：路径固定 `E:\obsidian\rein\<主题>\`，不与其他主题混用路径。
...
## 红线
1. 只允许在 `E:\obsidian\rein\` 下创建/修改文件；禁止触碰其他磁盘目录。
